<template>
  ...
  <div v-for="card in cards" :key="card.blockNum">
    <div class="card" v-bind:ref="card.blockNum">
      ...
      />
      <button v-if="card.tampered == true" @click="mine(card)" type="button">
        Mine
      </button>
    </div>
  </div>
...
const root_endpoint =
  process.env.VUE_APP_ROOT_ENDPOINT || "https://bc.dulcimer.live/go";
const ENDPOINT = `${root_endpoint}/single`;
